---
title: 🚀 跑项目闭环
weight: 4
bookCollapseSection: false
bookFlatSection: true
bookToc: false
noTocArea: true
---

## 项目实践：把“会用工具”变成“能交付成果”

面向有经验开发者的目标很简单：**用 AI 把一个项目从 0 跑到可交付**，并把过程沉淀成你自己的 SOP（下次更快、更稳）。

- **你会得到什么**  
  - 一条可复用的**项目闭环路径**（需求 → 拆解 → 实现 → 验证 → 交付 → 复盘）  
  - 一套更“项目级”的协作打法：**验证标准、节奏控制、上下文管理、重构与回归**  
  - 若干真实案例/配置集合：可直接抄作业，再按你的项目改

## 先从哪里开始？（按目标直达）

### 1) 我是新手，想尽快做完第一个闭环

- [新手端到端项目实战路径]({{< relref "project-practice/practices-two" >}})  
  这是“路线图”：把站内相关内容按端到端顺序串起来，适合先通读一遍建立地图。
- [Cursor 实战上手指南（10分钟）]({{< relref "project-practice/cursor" >}})  
  先把工具跑起来，避免第一天就卡在配置/订阅/环境上。

### 2) 我已经能写代码，但想把 AI 用到更稳（更少返工）

- [Claude Code 最佳实践]({{< relref "project-practice/best-practices" >}})  
  重点看：如何给验证标准、如何用 Plan Mode、如何拆分探索/规划/实现/提交。
- [Everything Claude Code]({{< relref "project-practice/everything-claude-code" >}})  
  一套生产级配置集合（rules/skills/hooks/agents/mcp），适合“准备长期打项目”的人照着落地。

### 3) 我想找个案例照着做一遍（先完成再优化）

- [项目实践：从一个具体案例开始]({{< relref "project-practice/practices-one" >}})  
  选一个规模可控的小项目，练习：需求拆解、任务分发、联调与验证闭环。
- [从需求到设计原型：一篇实战流程]({{< relref "project-practice/requirements-to-design-prototype" >}})  
  聚焦产品视角，从模糊想法到可交付的界面原型，把后续开发所需信息讲清楚。
- [AI 绘图实战：从文案到成品图]({{< relref "project-practice/ai-drawing-practice" >}})  
  聚焦视觉产出，用 AI 完成一整套场景配图，并把过程沉淀成可复用的绘图 SOP。

